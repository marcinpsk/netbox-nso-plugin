...
class TestMatchesDeviceValue(SimpleTestCase):
    ...
    def test_description_ignores_cosmetic_surrounding_whitespace(self):
        ...
        # these must not manufacture drift (regression: dev lab01d-rc1 et-0/0/6).
        self.assertTrue(matches_device_value("description", "*** IXIA TESTER 1/8", "*** IXIA TESTER 1/8 "))
        self.assertTrue(matches_device_value("description", "Core Link ***", " Core Link *** "))
    ...
    def test_stale_accepted_at_unowned_status_reads_drift(self):
        ...
        for status in ("imported", "unknown", "changed"):
            with self.subTest(status=status):
                ...
                self.assertEqual(
                    interface_row_state(st, _iface(description="lab01c-ri5.arcos - Core Link - unit")),
                    ("drift", "drift", False),
                )
