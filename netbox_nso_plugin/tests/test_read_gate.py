...
class TestOrchestratedOverwrites(_CascadeFlushMixin, TransactionTestCase):
    ...
    def test_publication_error_and_operator_edit_follow_canonical_lock_order(self):
        ...
        try:
            ...
            wait_until_postgres_blocks(operator_pid[0], "operator device-intent lock", locktype="advisory")
            self.assertFalse(
                operator_holds_level_four.is_set(),
                "the operator acquired the device-intent lock before the publication error released it",
            )
            self.assertFalse(
                operator_waits_for_level_five.is_set(),
                "the operator reached the management-row lock before the device-intent lock",
            )
            self.assertFalse(
        ...
        self.assertTrue(operator_waits_for_level_five.is_set(), "the operator did not reach the management-row lock")
