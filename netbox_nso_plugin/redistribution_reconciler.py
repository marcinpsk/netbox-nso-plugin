...
def _frozen_redistribution_operations(plan):
    ...
    models = {model._meta.label_lower: model for model in (Redistribution, NSORedistributionState)}
    creations = {}
...
def _reconcile_frozen_redistribution(writer):
    """Consume completed writes and execute the remaining frozen operations."""
    for operation, instance, update_fields, force_insert in _frozen_redistribution_operations(writer.plan):
        if operation == "delete":
            writer.delete(instance)
        elif not (writer.consume_existing_creation(instance) or writer.consume_applied_save(instance)):
            writer.save(instance, update_fields=update_fields, force_insert=force_insert)
