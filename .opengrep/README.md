...
| Persisted overlay signature used without validation | `nso-unguarded-overlay-signature` | Reports direct and module-qualified calls, plus simple assignment aliases of `_qualifying_overlay_signature`, outside `_valid_overlay_signature` and `_qualifying_overlay_signatures` in production Python modules. Calls through an alias and indirect references are not followed. |
