...
| Validation raised after the loop already skipped the entry | `nso-adapter-error-after-continue` | Reports every `raise` of `AdapterError` inside a `for` or `async for` loop whose body also holds a `continue`, at any nesting depth and in either statement order, in `netbox_nso_plugin/*_reconciler.py`. It accepts the fully qualified name, a package alias, a module alias, a plain module name, the bare symbol, and an aliased symbol, through absolute imports, and through relative imports at one, two, and three dots. The finding names the raise, not the loop header.<br>Limit: A relative import of a dotted module is level-exact. One, two, and three dots are spelled out; four dots or more are not matched. A dot-only relative import such as `from .. import adapter_client` matches every deeper level.<br>Limit: An imported name that a later statement rebinds or deletes still counts as `AdapterError`. This includes an assignment, a `del`, a loop target, a `with ... as` target, an `except ... as` target, and a later import of a different object. The retired guard dropped the binding at each of these.<br>Limit: A `*args` or `**kwargs` parameter does not suppress the rule. The retired guard treated it as a shadow.<br>Limit: A parameter that shadows the bound name suppresses the rule in the whole enclosing function, including a nested definition that rebinds the name with its own import. The retired guard applied the shadow only to the scope that declared it. A later import in the same function that rebinds the shadowed name does not re-enable the rule. The retired guard reported the fixture cases adapter_error_shadowed_alias and adapter_error_shadowed_module.<br>Limit: The import must precede the loop. A module-level import written below the function does not enable the rule, although the retired guard resolved it.<br>Limit: The import must be a statement of a block that encloses the loop. A module-level import inside a `with` block, or inside a `try` with no `except` clause, does not enable the rule, although the retired guard resolved it.<br>Limit: The fully qualified spelling is reported without a matching import statement. The retired guard needed a binding.<br>Limit: Unlike the retired AST guard, this rule does not match Unicode-normalized identifier spellings. |
