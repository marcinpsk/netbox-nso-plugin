...
# Shell functions for NetBox NSO Plugin development.
...
rq-jobs() {
  cd /opt/netbox/netbox && source /opt/netbox/venv/bin/activate && python manage.py shell -c     "from django_rq import get_queue; q = get_queue('default'); print(f'Jobs in queue: {len(q)}'); [print(f'  {job.id[:8]}: {job.func_name} - {job.get_status()}') for job in q.jobs[:10]]"
}

rq-failed() {
  cd /opt/netbox/netbox && source /opt/netbox/venv/bin/activate && python manage.py shell -c     "from django_rq import get_failed_queue; q = get_failed_queue(); print(f'Failed jobs: {len(q)}'); [print(f'  {job.id[:8]}: {job.func_name}') for job in q.jobs[:10]]"
}

rq-recent() {
  cd /opt/netbox/netbox && source /opt/netbox/venv/bin/activate && python manage.py shell -c     "from core.models import Job; jobs = Job.objects.all().order_by('-created')[:10]; [print(f'{j.id}: {j.name[:50]} - {getattr(j.status, "value", j.status)} ({j.user})') for j in jobs]"
}

dev-help() {
  echo "🎯 NetBox NSO Plugin Development Commands:"
  echo ""
